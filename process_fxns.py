...
def estimate_baseline_glucose(df_cgm, time_col="timestamp", glucose_col="glucose", percentile=30):
    if df_cgm.empty:
        return np.nan
...
def compute_metrics_for_all_windows(df_event_windows, df_cgm_data, df_events,baseline):
    ...
    for _, row in df_event_windows.iterrows():
        ...
        m = compute_window_metrics(df_cgm_data, start, end, baseline)
        metrics.append({
            **event_info, 
            **m,
            **m,
...
def time_in_zone(glucose_series):
    """
    Calculates percentage of time spent in defined glucose zones.
    
    Parameters:
        glucose_series (pd.Series): A numeric series of glucose values.
    
    Returns:
        dict: Zone name -> % time in that range (rounded to 1 decimal).
    """
